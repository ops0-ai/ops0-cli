...
func publishMessage(topic, message, key string, metadata map[string]interface{}) error {
	...
	// Create producer
	producer, err := kc.CreateProducer(kafkaAgent.producer)
	if err != nil {
	...
	var partition int32 = -1 // -1 means let Kafka choose
	...
	// Publish the message
	if err := kc.PublishMessage(producer, topic, message, key, partition, headers); err != nil {
		return err
	}

	// Log the operation
	logProducerOperation("publish", map[string]interface{}{
		"topic":     topic,
		"message":   message,
		"key":       key,
		"headers":   headers,
